...
pub fn PhysicalAddress(comptime architecture: Arch, comptime core_locality: CoreLocality) type {
    const Usize = lib.TargetUsize(architecture);
    return enum(Usize) {
        _,

        const PA = @This();
        const VA = VirtualAddress(architecture, core_locality);
...
pub fn VirtualAddress(comptime architecture: Arch, comptime core_locality: CoreLocality) type {
    const Usize = lib.TargetUsize(architecture);
    return enum(Usize) {
        null = 0,
        _,

        const PA = PhysicalAddress(architecture, core_locality);
        const VA = @This();
...
pub fn PhysicalMemoryRegion(comptime architecture: Arch, comptime core_locality: CoreLocality) type {
    const Usize = lib.TargetUsize(architecture);
    ...
    return extern struct {
        ...
        const PA = PhysicalAddress(architecture, core_locality);
        const PMR = @This();
        const VMR = VirtualMemoryRegion(architecture, core_locality);

        pub inline fn new(address: PA, size: Usize) PMR {
            return .{
                .address = address,
                .size = size,
            };
...
pub fn VirtualMemoryRegion(comptime architecture: Arch, comptime core_locality: CoreLocality) type {
    const Usize = lib.TargetUsize(architecture);

    return extern struct {
        address: VirtualAddress(architecture, core_locality),
        size: Usize,

        const PMR = PhysicalMemoryRegion(architecture, core_locality);
        const VMR = @This();
...
pub fn PhysicalAddressSpace(comptime architecture: Arch) type {
    _ = architecture;
    return extern struct {
        free_list: List = .{},
        heap_list: List = .{},
...
pub fn ArchPhysicalAddress(comptime architecture: Arch) fn (comptime core_locality: CoreLocality) type {
    return extern struct {
        fn function(comptime core_locality: CoreLocality) type {
            return PhysicalAddress(architecture, core_locality);
        }
    }.function;
}

pub fn ArchVirtualAddress(comptime architecture: Arch) fn (comptime core_locality: CoreLocality) type {
    return extern struct {
        fn function(comptime core_locality: CoreLocality) type {
            return VirtualAddress(architecture, core_locality);
        }
    }.function;
}

pub fn ArchPhysicalMemoryRegion(comptime architecture: Arch) fn (comptime core_locality: CoreLocality) type {
    return extern struct {
        fn function(comptime core_locality: CoreLocality) type {
            return PhysicalMemoryRegion(architecture, core_locality);
        }
    }.function;
}

pub fn ArchVirtualMemoryRegion(comptime architecture: Arch) fn (comptime core_locality: CoreLocality) type {
    return extern struct {
        fn function(comptime core_locality: CoreLocality) type {
            return VirtualMemoryRegion(architecture, core_locality);
        }
    }.function;
}
