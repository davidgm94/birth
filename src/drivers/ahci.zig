...
comptime {
    std.assert(@bitSizeOf(Disk.Operation) == @bitSizeOf(u1));
    std.assert(@enumToInt(Disk.Operation.read) == 0);
    std.assert(@enumToInt(Disk.Operation.write) == 1);
}
...
pub const Drive = struct {
    ...
    fn configure(drive: *Drive, virtual_address_space: *VirtualAddressSpace) void {
        ...
        defer drive.start_command();
        {
            ...
            const command_list_alloc_result = virtual_address_space.allocate_extended(arch.page_size, null, .{ .write = true }, .no, false, 0) catch @panic("wtf");
            // TODO: what's 1024?
            ...
            const fis_alloc_result = virtual_address_space.allocate_extended(arch.page_size, null, .{ .write = true }, .no, false, 0) catch @panic("Wtf");
            ...
            const command_table_address = virtual_address_space.allocate_extended(arch.page_size, null, .{ .write = true }, .no, false, 0) catch @panic("Wtf");
    ...
    const FISRegisterHardwareToDevice = extern struct {
        ...
        comptime {
            std.assert(@sizeOf(FISRegisterHardwareToDevice) == 5 * @sizeOf(u32));
        }
    };
    ...
    const HBAPRDTEntry = packed struct {
        ...
        comptime {
            std.assert(@sizeOf(HBAPRDTEntry) == @sizeOf(u64) * 2);
        }
    };
    ...
    const HBACommandTable = struct {
        ...
        comptime {
            std.assert(@sizeOf(HBACommandTable) == @sizeOf(u8) * 128);
        }
    };
...
const HBACommandHeader = extern struct {
    ...
    comptime {
        std.assert(@sizeOf(HBACommandHeader) == 8 * @sizeOf(u32));
    }
};
