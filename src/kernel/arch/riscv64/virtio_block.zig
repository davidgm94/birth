...
pub const Device = struct {
    ...
    fn access(self: *@This(), buffer: []u8, size: u64, offset: u64, operation: Operation, watcher: u16) void {
        ...
        // TODO @Heap
        kernel.assert(@sizeOf(Request) < kernel.arch.page_size);
        const request = @intToPtr(*Request, kernel.arch.physical.allocate_pages(1) orelse @panic("unable to allocate for block request\n"));
        {
            ...
            const head_index = self.fill_next_descriptor(descriptor);
            request.header.sector = sector;
            request.header.block_type = if (write) BlockType.out else BlockType.in;
...

        {
            const descriptor = virtio.Descriptor {
                ...
                .len = size,
                .flags = virtio.Descriptor.Flags.next | if (!write) virtio.Descriptor.Flags.write else 0,
                .next = 0,
            };
        ...
        self.queue.available.ring[self.queue.avaiable.index % virtio.ring_size] = head_index;
        self.queue.avaiable.index = self.queue.available.index +% 1;

        @intToPtr(*volatile u32, self.address + virtio.Offset.queue_notify).* = 0;
        return size;
    }
