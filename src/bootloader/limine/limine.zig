...
fn mapSection(bootloader_information: *bootloader.Information, minimal_paging: paging.Specific, comptime section_name: []const u8, flags: VirtualAddressSpace.Flags) !void {
    ...
    minimal_paging.map(physical_address, virtual_address, section_size, flags, .bootloader) catch @panic("Mapping of section failed");
...
pub fn entryPoint() callconv(.C) noreturn {
    ...
    for (bootloader_information.getMemoryMapEntries()) |entry| {
        if (entry.type == .usable) {
            ...
            minimal_paging.map(entry.region.address, entry.region.address.toIdentityMappedVirtualAddress(), lib.alignForwardGeneric(u64, entry.region.size, lib.arch.valid_page_sizes[0]), .{ .write = true, .execute = true }, .bootloader) catch @panic("Mapping of usable memory map entry failed");
            minimal_paging.map(entry.region.address, entry.region.address.toHigherHalfVirtualAddress(), lib.alignForwardGeneric(u64, entry.region.size, lib.arch.valid_page_sizes[0]), .{ .write = true, .execute = true }, .bootloader) catch @panic("Mapping of usable memory map entry failed");
        }
    }
    ...
    minimal_paging.map(framebuffer_physical_address, framebuffer_virtual_address, lib.alignForwardGeneric(u64, bootloader_information.framebuffer.pitch * bootloader_information.framebuffer.height, lib.arch.valid_page_sizes[0]), .{ .write = true, .execute = false }, .bootloader) catch @panic("can't map framebuffer");
    ...
    for (memory_map_entries) |entry| {
        if (entry.type == .bootloader_reclaimable) {
            if (entry.region.address.toHigherHalfVirtualAddress().value() < rsp and entry.region.address.offset(entry.region.size).toHigherHalfVirtualAddress().value() > rsp) {
                minimal_paging.map(entry.region.address, entry.region.address.toHigherHalfVirtualAddress(), entry.region.size, .{ .write = true, .execute = false }, .bootloader) catch @panic("Mapping of bootloader information failed");
                break;
            }
        }
