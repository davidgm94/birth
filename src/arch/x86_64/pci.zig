...
pub fn read_config(comptime T: type, bus: u8, slot: u8, function: u8, offset: u8) T {
    lock.acquire();
    defer lock.release();
...
pub fn write_config(comptime T: type, value: T, bus: u8, slot: u8, function: u8, offset: u8) void {
    lock.acquire();
    defer lock.release();
