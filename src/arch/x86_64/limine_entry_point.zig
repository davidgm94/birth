...
const PhysicalMemoryRegion = rise.PhysicalMemoryRegion;
...
pub export fn kernel_entry_point() noreturn {
    ...
    // Sanity check the higher_half value
    {
        ...
        if (response.offset != kernel.higher_half) rise.panic("Unexpected offset: 0x{x}", .{response.offset});
    }
...
pub fn panic(message: []const u8, _: ?*common.StackTrace, _: ?usize) noreturn {
    rise.panic_extended("{s}", .{message}, @returnAddress(), @frameAddress());
}
