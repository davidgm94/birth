...
pub fn safe_function_cast(function: anytype, comptime parameters: SafeFunctionCastParameters) SafeFunctionCastError!parameters.FunctionType {
    if (is_comptime()) {
        ...
        switch (parameters.match_return_type) {
            .crazytown => @panic("wtf"),
            .size => {
                if (source_function_type.return_type != destination_function_type.return_type) {
            ...
            .type => {
                if (source_function_type.return_type != destination_function_type.return_type) return SafeFunctionCastError.return_type_mismatch;
            },
        }

        for (source_function_type.args) |source_argument, arg_i| {
            const destination_argument = destination_function_type.params[arg_i];
            ...
            switch (parameters.match_argument_types) {
                .crazytown => @panic("wtf"),
                .size => {
                    if (source_argument.arg_type != destination_argument.arg_type) {
                        const source_bit_size = @bitSizeOf(source_argument.arg_type);
                        const destination_bit_size = @bitSizeOf(destination_argument.arg_type);
